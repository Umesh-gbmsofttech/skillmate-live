...
public interface BatchRepository extends JpaRepository<Batch, Long> {
        ...
        // batch_id = :batchId);
        // DELETE FROM meeting WHERE batch_id = :batchId;
        // DELETE FROM batch_students WHERE batch_id = :batchId;
        // DELETE FROM batch WHERE id = :batchId;
        // """, nativeQuery = true)
        // void deleteByBatchId(@Param("batchId") Long batchId);
        @Modifying
        @Transactional
        @Query(value = "DELETE FROM attendance WHERE meeting_id IN (SELECT id FROM meeting WHERE batch_id = :batchId)", nativeQuery = true)
        void deleteAttendanceByBatchId(@Param("batchId") Long batchId);

        @Modifying
        @Transactional
        @Query(value = "DELETE FROM meeting WHERE batch_id = :batchId", nativeQuery = true)
        void deleteMeetingsByBatchId(@Param("batchId") Long batchId);

        @Modifying
        @Transactional
        @Query(value = "DELETE FROM batch_students WHERE batch_id = :batchId", nativeQuery = true)
        void deleteBatchStudentsByBatchId(@Param("batchId") Long batchId);

        @Modifying
        @Transactional
        @Query(value = "DELETE FROM batch WHERE id = :batchId", nativeQuery = true)
        void deleteBatchById(@Param("batchId") Long batchId);
